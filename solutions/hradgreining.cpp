...
int main(int argc, char* argv[]) {
  ...
  str word;
  read(word);
#define DNA_STRING "COV"
  int ok = 0;

  // Subtract last two letters, as they was checked in prev iteration, and
  // cannot do substring with 3 in the end.
  for (int i = 0; i < word.length() - 2; i++) {
    ok |= word.substr(i, 3) == DNA_STRING;
  }

  IF(ok, print("Veikur!"), print("Ekki veikur!"));
