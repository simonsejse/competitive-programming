...
int main(int argc, char* argv[]) {
  ...
  ll_read(a);
  ll prices[a];
  f(i, 0, a) { read(prices[i]); }

  ll reimburse   = *max_element(prices, prices + a) / 2;
  ll best_flight = *min_element(prices, prices + a);
  ll min_amount  = best_flight - reimburse;
  IF(min_amount < 0, print(0), print(min_amount));
  return 0;
}
