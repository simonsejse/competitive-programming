...
template<typename T>
bool In(const T& x, const std::set<T>& s) {
    return s.find(x) != s.end();
}

template<typename T>
bool In(const T& x, const std::vector<T>& v) {
...
auto toset(const U& v) -> std::set<typename U::value_type> {
    return set<typename U::value_type>(v.begin(), v.end());
}
...
int main(int argc, char* argv[]) {
  ...
  vc vowels = {'a', 'e', 'i', 'o', 'u'};
  f(i, 0, w.size()) {
    if (find(vowels.B, vowels.E, w[i]) != vowels.end()) {
      v += 1;
    }
    if (w[i] == 'y')
      nv += 1;
